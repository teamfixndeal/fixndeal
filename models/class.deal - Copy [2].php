...
/*	function buymembership($mid,$price,$day){
	...
	$statusListf=$db->getRows("select * from ".POSTS." where added_by='".$id."' and status=3 ");
...
//	$statusList=$db->getRows("select r.*,u.*,p.* from ".REVIEWS." as r,".SITE_USER." as u,".POSTS." as p where r.reviewfor='".$id."' and u.username='".$userid."' and  p.added_by='".$id."'");
		...
		$status['user']=$db->getRow("select * from ".SITE_USER." where id='".$id."'");

		$status['user']['urat']=$this->getAvgRating($id);
...
return $ProDetail;
	...
	function getProducDetail($id){
		...
		SELECT p.*,u.udate as joinDate,u.username,u.show_photo,u.show_number,u.id as userid,u.fullname,u.email,u.mobile,u.avatar,u.mob_verify
